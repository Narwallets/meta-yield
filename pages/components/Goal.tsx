...
export const Goal = (props: GoalProps) => {
  const { isActive, isCompleted, isLastGoal, isFirstGoal, kickstarterGoal, ...stackProps } =
    props
    ...
    <Stack spacing="4" direction={{ base: 'row', md: 'column' }} flex="1" {...stackProps}>
      <Stack spacing="0" align="center" direction={{ base: 'column', md: 'row' }}>
        ...
        <Divider
          orientation={orientation}
          borderWidth="1px"
          borderColor={isCompleted ? 'accent' : isLastGoal ? 'transparent' : 'inherit'}
        />
      </Stack>
      <Stack
        spacing="0.5"
      ...
      >
        ...
        <Text color="emphasized" fontWeight="medium">
          ${kickstarterGoal.desired_amount/10**24}
        </Text>
      </Stack>
    </Stack>
  )
